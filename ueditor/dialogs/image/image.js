...
//                        });
     ...
     function getImage() {
      ...
      ajax.request(editor.options.imageManagerUrl, {
        ...
        var attachments = eval('('+xhr.responseText+')'); 
        g("imageList").innerHTML = !attachments.length ? "&nbsp;&nbsp;" + lang.noUploadImage : "";

        var attachment;
        for (var k = 0, at; at = attachments[k++];) {
                ...
                img.setAttribute(k < 35 ? "src" : "lazy_src", at.Url);
                img.setAttribute("data_ue_src", at.Url);
                img.setAttribute("style", "margin:0 auto; ");

                var button = document.createElement("a");
                button.innerHTML = "删除";
                button.setAttribute("style", "display:block; width:100px; margin:auto; text-decoration:none; text-align:center; color:#000; border:solid 1px #D7D7D7; background:#EEE;");
                button.setAttribute("href", "javascript:void(0)");
                button.setAttribute("versionNo", at.VersionNo);
                button.onclick = function () 
                {
                    ajax.request("/Common/Attachment/Remove?TypeKey=" + attachment.TypeKey + "&Key=" + attachment.Key + "&VersionNo=" + this.getAttribute("versionNo"), 
                    {
                        timeout:100000,
                        action:"get",
                        onsuccess:function () 
                        ...
                        onerror:function () {g("imageList").innerHTML = lang.imageLoadError;}
                    });
                };

                 var divImag = document.createElement("p");
                divImag.setAttribute("style", "height: 78px; margin:0 auto 3px auto; overflow: hidden;");
                divImag.appendChild(img);

                var div = document.createElement("div");
                div.setAttribute("style", "width: 115px; height:100px; text-align:center;");
                div.appendChild(divImag);
